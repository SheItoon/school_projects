...
inline sw::Matrix<T> operator*(const sw::Matrix<T>& left, const sw::Matrix<T>& right)
{
    ...
    return (sw::Matrixf(resultFlist));
}

template <typename T>
inline sw::Matrix<T>& operator*(const sw::Matrix<T>& left, T right)
{
    for (unsigned line_ctr = 0; line_ctr < left.line(); line_ctr += 1)
        for (unsigned col_ctr = 0; col_ctr < left.col(); col_ctr += 1)
            left.m_matrix[line_ctr][col_ctr] *= right;
    return (left);
}
...
template <typename T>
inline sw::Matrix<T>& operator*=(sw::Matrix<T>& left, T right)
{
    for (unsigned line_ctr = 0; line_ctr < left.line(); line_ctr += 1)
        for (unsigned col_ctr = 0; col_ctr < left.col(); col_ctr += 1)
            left.m_matrix[line_ctr][col_ctr] *= right;
